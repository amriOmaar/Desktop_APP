...
import ijacampi.Services.MoyenTransportService;
import ijacampi.Services.Offre_LocationService;
import ijacampi.Services.User_service;
import ijacampi.Tests.MyListenerTran;
...
public class OffreInterfaceController implements Initializable {
    ...
    private MyListenerTran MyListenerTran;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            if (MoyenTransportService.afficher().size() > 0) {
                ...
                MyListenerTran = new MyListenerTran() {
        ...
        try {
            for (int i = 0; i < MoyenTransportService.afficher().size(); i++) {
                ...
                OffreCardController.setData(MoyenTransportService.afficher().get(i), MyListenerTran);
                if (column == 3) {
                    column = 0;
                    row++;
                }
